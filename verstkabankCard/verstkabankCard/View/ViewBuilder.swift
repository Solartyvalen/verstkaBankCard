...
final class ViewBuilder: NSObject {
    ...
    private let maneger = ViewManeger.shared
    ...
    var cardColor: [String] = ["#16A085FF", "#003F32FF"] {
        willSet {
            if let colorView = view.viewWithTag(7) {
                ...
                let gradient = maneger.getGradient(colors: newValue)
                colorView.layer.insertSublayer(gradient, at: 0)
            }
        }
    }
    var cardIcon: UIImage = UIImage(resource: .icon1) {
        willSet {
    ...
    func getCard() {
       ...
       card = maneger.getCard(colors: cardColor, balance: balance, cardNumber: cardNumber, cardImage: cardIcon)
    ...
    func getColorSlider() {
        ...
        let colorTitle =  maneger.slideTitle(titleText: "Select color")
        
        colorCollection = maneger.getCollection(id: RestoreIDs.colors.rawValue, dataSourse: self, delegate: self)
    ...
    func setIconSlider() {
        let iconSliderTitle = maneger.slideTitle(titleText: "Add shapes")
        
        imageCollection = maneger.getCollection(id: RestoreIDs.images.rawValue, dataSourse: self, delegate: self)
    ...
    func setDescriptionText() {
        let descrText: UILabel = {
            let text = UILabel()
            text.text = "Don't worry. You can always change the design of your virtual cardlater. Just enter the settings."
...
extension ViewBuilder: UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        ...
        case RestoreIDs.colors.rawValue:
            return maneger.colors.count
        case RestoreIDs.images.rawValue:
            return maneger.images.count
        default:
            return 0
        }
    }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        case RestoreIDs.colors.rawValue:
            if let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "cell", for: indexPath) as? ColorCell {
                let color = maneger.colors[indexPath.item]
        ...
        case RestoreIDs.images.rawValue:
            if let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "cell",
                ...
                let icon = maneger.images[indexPath.item]
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        switch collectionView.restorationIdentifier {
        case RestoreIDs.colors.rawValue:
            let colors = maneger.colors[indexPath.item]
        ...
        case RestoreIDs.images.rawValue:
            let image = maneger.images[indexPath.item]
            self.cardIcon = image
